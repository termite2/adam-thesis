...
Given a valuation of a subset of the concrete state variables $V \subseteq C$ we say that the valuation \emph{agrees} with a concrete state $c$ if the value assigned to each of the variables in $V$ equals the value assigned to the same variable by $c$. A set of variables defines a partitioning of the concrete state space: 
...
When solving a reachability game, we refine at the \emph{may-must boundary}. As explained in section \ref{sec:threeval_generic}, we split an abstract state that is may-winning and for which it is possible to force execution into $W^M$ from a subset of the concrete states within the abstract state. We now describe how to do this efficiently and symbolically.

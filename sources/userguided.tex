...
\end{aligned}
$$
where $\mathtt{reg\_dat'}$ is the next-state variable representing the value of $\mathtt{reg\_dat}$ after the transition, and$\mathtt{tag}$ and $\mathtt{v}$ are controllable action variables, where $\mathtt{tag}$ models the method being invoked, and $\mathtt{v}$ is the argument of the method.
        
%    \item Controllable action variables $Y_c$ encode controllable 
%        method invocations available to the driver.  A special        
%        $tag$ variable is used to identify the method being 
%        invoked.  For example, the specification in 
%        Figure~\ref{f:ex} has four controllable methods, so $tag$ 
%        can take one of the four values $\{write\_dat, 
%        write\_cmd, read\_status, send\_ack\}$.  In addition, a 
%        separate variable is introduced for each argument of 
%        every controllable method.  
%        
%        i.e., when the driver calls the $write\_dat$ method, 
%        $reg\_dat$ takes the value of argument $v$  and remains 
%        unchanged otherwise.        
...
%Extracting atomic uncontrollable transitions is slightly more 
%complicated.  In \tsl all uncontrollable transitions happen in 
%the context of processes.  We decompose each process into atomic 
%transition.  An atomic transition consists of all code statements 
%between two \emph{pause locations}.  A pause location is a 
%\src{wait} statement or a magic block.  Example (psend process).  
%Thus, environment action is fully determined by the choice of one 
%of the enabled processed to execute and any nondeterministic 
%values that occur in the process.  So we introduce an 
%uncontrollable action variable for pid and each nondet value.  
%Once we've decomposed processes into atomic transitions, we can 
%now define variable update functions. Example: var update for 

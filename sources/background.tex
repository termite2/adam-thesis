...
Most operating systems are multithreaded, including the device drivers. Driver entry points and interrupt handlers may be invoked concurrently and in a nested fashion. Careful use of synchronization primitives are required to avoid race conditions and deadlock. In this work, we assume that only one driver entry point is invoked at a time, removing all concurrency. This can be achieved by using an event based framework such as Dingo \ref{dingo}, writing a wrapper to translate the concurrent interface required by the operating system to the single threaded interface of the generated drivers, or by post processing the generated driver by a tool such as (something) \cite{concurrent_drivers}
...
We give the algorithm for computing the set of states for which a $\mu$-calculus formula holds in Algorithm~\ref{a:mu_semantics}. It assumes the existence of an abstract set datatype that supports set union, complementation and equality checking.
...
\Function{MuSemantics}{$\phi$}
    ...
    \ElsIf {$\phi = \forall X. \psi$}
        \State\Return $\Call{UniversallyQuantify}{X, \psi}$
    \ElsIf {$\phi = \nu X. \psi$}
        \State $Z \gets S$
        \Loop
...
\todo{$CPre$ isnt the order preserving function}

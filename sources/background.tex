...
A device driver is typically dynamically linked in to the running operating system kernel, as is commonly the case with monolithic designs. Or, it may be contained within its own address space as a standalone process, as is common in microkernel designs. Either way, once it starts up it registers the services it provides with the operating system and, possibly in response to an OS request, it detects and initialises the device. From then on, until the device or the driver are removed, it is responsible for translating requests between the OS and the hardware device.
...
Most operating systems are multithreaded, including the device drivers. Driver entry points and interrupt handlers may be invoked concurrently and recursively. Careful use of synchronisation primitives are required to avoid race conditions and deadlock. In this work, I assume that only one driver entry point is invoked at a time, removing all concurrency. This includes interrupts, which are queued until after any currently executing entry point completes. This can be achieved by using an event based framework such as Dingo \cite{Ryzhyk_CKH_09}, writing a wrapper to translate the concurrent interface required by the operating system to the single threaded interface of the generated drivers, or by post processing the generated driver as described by \cite{Cerny_HRRT_13, sched-cav, scheduling-cav15}

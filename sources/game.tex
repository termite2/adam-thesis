...
The device and OS specifications are combined by taking their product, which means instantiating them in parallel and synchronising them on the shared class events. If $D$ is the set of device states and $O$ is the set of OS states, then $D \times O$ is the set of states in the combined specification. If $\lambda_D$ is the set of device labels and $\lambda_O$ is the set of OS labels, then $\lambda_D \cup \lambda_O$ is the set of labels in the combined specification. The device specification is extended so that all events in $\lambda_O \setminus \lambda_D$ are self loops in all states. The OS specification is extended similarly. If the transitions $\langle s_1, \lambda, d_1 \rangle$ and $\langle s_2, \lambda, d_2 \rangle$ exist in the device and OS state machines respectively, then the transition $\langle (s_1, s_2), \lambda, (d_1, d_2) \rangle$ exists in the combined specification.

...
It addresses the usability challenge by generating driver code in a user guided fashion. Termite acts as an advanced auto-complete, suggesting lines of driver code while simultaneously verifying hand written ones. Termite's specifications are written in a familiar imperative style and Termite provides a comprehensive counterexample guided debugging facility for use when the specifications contain defects that prevent Termite from synthesising a driver.

It addresses the scalability challenge with a new scalable synthesis engine which enables it to handle real-world device specifications that are beyond the reach of existing synthesis techniques. This thesis presents the predicate-based abstraction-refinement engine at the heart of Termite. Solutions are developed to the three key problems involved in the implementation of efficient predicate abstraction which have not been addressed previously in a synthesis settings: (1) keeping abstractions concise by identifying relevant predicates only, (2) solving abstract games efficiently, (3) and computing abstractions symbolically. 

The proposed techniques are implemented in the Termite tool and evaluated by synthesising drivers for a number of I/O devices representative of a typical embedded platform. Termite is the first tool to combine the power of automation with the flexibility of conventional development. It is also the first practical synthesis tool based on abstraction refinement. Finally, it is the first synthesis tool to support automated debugging of input specifications.

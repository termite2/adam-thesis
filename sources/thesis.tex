...
At the core of \termite is a scalable synthesis engine, which enables it to handle real-world device specifications that are beyond the reach of existing synthesis techniques. We present the predicate-based abstraction refinement algorithm that this synthesis engine is based on. We develop solutions to the key problems involved in implementing efficient predicate abstraction, which previously have not been addressed in synthesis settings: (1) keeping abstractions concise by identifying relevant predicates only, (2) solving abstract games efficiently, and (3) computing and solving abstractions symbolically. 

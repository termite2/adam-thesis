...
\newcommand{\mucalc}{fixed point\xspace}
...
Device drivers are hard and tedious to write, and they are notorious for being unreliable. Automatic device driver synthesis is a radical approach to creating drivers faster and with fewer defects by generating them automatically based on operating system and hardware device specifications. This thesis addresses the two fundamental challenges in driver synthesis: (1) usability and (2) scalability.

The usability challenge is to enable developers to create drivers through a well-defined, predictable sequence of steps.  It falls into the following three design objectives: (1) ease of developing specifications for driver synthesis, (2) ease of debugging synthesis errors due to incorrect specifications, and (3) enabling the benefits of automation without sacrificing the flexibility of conventional driver development.  In this thesis I develop a new driver synthesis methodology that addresses the first objective through a high-level specification language whose syntax and semantics are close to those of familiar imperative programming languages like C.  In addition, it facilitates the development of modular specifications and thus enables a high degree of specification reuse.  I address the second objective by developing powerful debugging techniques that help the developer identify and fix specification defects through well-defined process, similar to how conventional debuggers help troubleshoot implementation errors.  Finally, the proposed methodology strikes the balance between automation and flexibility by generating driver code in a user guided fashion, where the user can interactively alter or amend automatically generated code, while the synthesis tool verifies user changes on the fly.

%Termite acts as an advanced auto-complete, suggesting lines of driver code while simultaneously verifying hand written ones. Termite's specifications are written in a familiar imperative style and Termite provides a comprehensive counterexample guided debugging facility for use when the specifications contain defects that prevent Termite from synthesising a driver.

The scalability challenge is to overcome the performance bottleneck of existing synthesis algorithms that are not able to efficiently handle real-world device specifications.  I address this challenge with a new scalable synthesis engine based on predicate-based abstraction-refinement.  Solutions are developed to the three key problems involved in the implementation of efficient predicate abstraction which have not been addressed previously in a synthesis settings: (1) keeping abstractions concise by identifying relevant predicates only, (2) solving abstract games efficiently, (3) and computing abstractions symbolically. 
...
\begin{itemize}
    ...
    \item \bibentry{syntcomp_arxiv}
    \item \bibentry{CAV_nocpre}
    \item \bibentry{Vij_KRHZRWL_13}
\end{itemize}

{\let\clearpage\relax\chapter*{Awards}}
\begin{itemize}
    \item Reactive synthesis competition 2014: Kurt G{\"o}del Medal in Silver. My solver placed first in the sequential realizability track.
    \item Reactive synthesis competition 2015: Kurt G{\"o}del Medal in Silver. My solver placed first in the sequential realizability track.
\end{itemize}
...
\bibliography{bibtex/defs.bib,bibtex/combined.bib,extra.bib}

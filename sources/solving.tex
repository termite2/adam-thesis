...
The three valued variable abstraction refinement loop is given in Algorithm~\ref{alg:three_val_abs_ref} and the procedure to refine the abstraction is given in Algorithm~\ref{alg:refineAbstractionVar}.
...
The algorithm begins in the same way as for safety and reachability. We solve the game with the current abstraction to find $W^m$ and $W^M$ on lines~\ref{a:tvmc:sM} and~\ref{a:tvmc:sm} and terminate if these allow us to determine the outcome on lines~\ref{a:tvmc:Y} and~\ref{a:tvmc:N}. We then refine and solve again. Again, as in the reachability and safety cases, we aim to grow $W^M$ and shrink $W^m$ so that, if we do not terminate early, they will eventually become the same set. This guarantees termination through one of the if conditions. Variable promotion is the same as before but operates on different boundary states. We describe the algorithm to find these boundaries. 
...
To address the second issue, I modify the algorithm to handle a set of abstract labels at every iteration.  To this end, in line~\ref{a:refineCpre:pickSat}, instead of choosing a complete assignment to state, untracked, and label predicates, I compute a \emph{prime implicant} of set $T$, i.e., an assignment to a subset of variables in $\vect{\sigma}\cup\vect{\omega}\cup\vect{\lambda}$ such that any extension of this assignment to the remaining variables satisfies $T$: 
$$

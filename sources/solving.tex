...
Additionally, these update functions may depend on a subset of the label variables. In our running example, it is possible to set $U$ to the value of $L$, a label variable, and, each variable also implicitly depends on the $tag$ variable. These Boolean label variables that the the update functions for state variables in $V_\alpha$ depend on are denoted $\Lambda$. Note that in our running example $U \notin V_\alpha$ initially, so initially $\Lambda$ is just $\{tag\}$.

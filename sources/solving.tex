...
This is jut one of the potential pitfalls of handling predicate abstraction in the same way as variable abstraction. The goal of the following sections is to apply the machinery developed in the proceeding sections to predicate abstraction, so we must identify these pitfalls and take them into account.

\subsubsection{Algorithm}

Following \cite{three_val_abs_ref}, we modify the algorithm to work on approximate versions of the controllable predecessor operators, $\cpremp \text{ and} \cpreMm :: 2^V \rightarrow 2^V$. In the following sections we will show how to efficiently compute these operators. We require that, for all $U \subseteq V$ we have:

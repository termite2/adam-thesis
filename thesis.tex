...
% \Function{Solve}{$transitionRelation$, $goal$}
    \Statex {\bf Input:} A game structure $G = \langle S, L, I, \tau_1, \tau_2, \delta \rangle$, a set of sets of target states $T\subseteq 2^S$, a set of sets of fair states $F\subseteq 2^S$, and an initial abstraction $\alpha=\langle V, \concrete{} \rangle$ that is precise for $T$, $F$, $I$, and $\tau_i$.

    \Statex {\bf Output:} {\it Yes} if $I \subseteq \textsc{Buchi}(T, F, Cpre_1)$, and {\it No} otherwise.

    \Loop
    \State $W^M \gets \textsc{Buchi}(\abstractM{T}, \abstractM{F}, Cpre_1^M)$
    \State $W^m \gets \textsc{Buchi}(\abstractm{T}, \abstractm{F}, Cpre_1^m)$
        \If{$\abstractM{I} \subseteq W^M$} 
            \State\Return Yes
        \ElsIf{$\abstractM{I} \nsubseteq W^m$} 
